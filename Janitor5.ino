...
int difficulty = 30; //subtracted from frequency after each task completed

long unsigned frequency = 3000; //how often tasks get assigned in milliseconds

long unsigned limit = 500; //lower limit to frequency

long unsigned taskTime = 5000; //time given to complete a task
...
int threshold = 520; //threshold for photocell
...
//bool ventChange = false;
int prevStatus = ventVal;

bool checkVent()
{
  if (prevStatus != ventVal)
  {
    prevStatus = ventVal;
    return true;
  }
  else //prevStatus == ventVal
  {
    return false;
  }
}
...
open_close = true; //vent open true or closed false

faucetRand = 0; //random faucet value

plunges = 0; //tracks plunging multiple times
randPlunges = 5;
plungeOK = true;

sprays = 0; //tracks spraying mulitple times
randSprays = 5;
sprayOK = true;

swats = 0; //tracks swatting multiple times
randSwats = 5;
swatOK = true;
...
//task checker
  ...
  if (vent) //vent task is active
  {
    ...
    else //still have time
    {
      ...
      if (checkVent()) //returns true if 
      {
        vent = false;
        if (open_close == true) //vent is now closed
        {
          open_close = false;
          Keyboard.print("j"); //press j
        }
